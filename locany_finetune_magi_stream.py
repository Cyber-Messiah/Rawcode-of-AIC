...
MODALITY_NAME_TO_ID = {"rgb": 0, "thermal": 1, "depth": 2}
...
class LazySupervisedDatasetMTP(Dataset):
    ...
    def __init__(self,
                 ...
                 use_modality_fusion: bool = False,
        ...
        if not 0.0 <= self.modality_dropout_prob <= 1.0:
    ...
    def _load_auxiliary_images(self, auxiliary_entries: list) -> list:
        loaded = []
        for entry in auxiliary_entries:
            modality = str(entry.get("modality", "")).lower()
            if modality not in ("thermal", "depth"):
                raise ValueError(f"Unsupported auxiliary modality: {modality}")
            if random.random() < self.modality_dropout_prob:
                continue
            path = entry.get("path")
            if not path:
                raise ValueError(f"Missing path for {modality} auxiliary image.")
            full_path = path if osp.isabs(path) else osp.join(self.root, path)
            with Image.open(full_path) as image:
                loaded.append((modality, image.convert("RGB")))
        return loaded

    def multi_modal_get_item(
            self,
            messages: list,
            auxiliary_images: Optional[list] = None,
    ) -> Dict[str, torch.Tensor]:
        auxiliary_images = auxiliary_images or []
        ...
        if image_inputs is not None:
            image_inputs = [
                apply_resize_augmentation(
                    img, data_augment=self.data_augment and not auxiliary_images,
        ...
        if "pixel_values" not in inputs:
            ...
            modality_group_sizes = torch.tensor([1], dtype=torch.long)
        else:
            pixel_values = inputs["pixel_values"]
            image_grid_hws = inputs["image_grid_hws"]
            self._validate_image_token_alignment(input_ids, pixel_values, image_grid_hws)
            if auxiliary_images and len(image_grid_hws) != 1:
                raise ValueError("Feature fusion currently requires exactly one RGB anchor image.")

            modality_ids_list = [MODALITY_NAME_TO_ID["rgb"]]
            aux_pixel_values = []
            aux_grids = []
            for modality, image in auxiliary_images:
                aux_inputs = self.processor.image_processor(
                    images=[image], return_tensors="pt"
                )
                aux_pixel_values.append(aux_inputs["pixel_values"])
                aux_grids.append(aux_inputs["image_grid_hws"])
                modality_ids_list.append(MODALITY_NAME_TO_ID[modality])

            if aux_pixel_values:
                pixel_values = torch.cat([pixel_values, *aux_pixel_values], dim=0)
                image_grid_hws = np.concatenate([image_grid_hws, *aux_grids], axis=0)

            # One fused visual group occupies only the RGB image-token span.
            image_flags = torch.tensor([1], dtype=torch.long)
            modality_ids = torch.tensor(modality_ids_list, dtype=torch.long)
            modality_group_sizes = torch.tensor(
                [len(modality_ids_list)], dtype=torch.long
            )
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        while retry_count <= 10:
            ...
            try:
                ...
                auxiliary_entries = data_item.pop("auxiliary_images", [])
                if auxiliary_entries and not self.use_modality_fusion:
                    raise ValueError(
                        "Dataset contains auxiliary_images but --use_modality_fusion is disabled."
                    )
                auxiliary_images = self._load_auxiliary_images(auxiliary_entries)
                data_item = process_multimodal_sample(
                    data_item, self.root, self.max_frames, 
                    self.target_fps, self.video_total_pixels,
                    visual_prompt=self.visual_prompt,
                )
                return self.multi_modal_get_item(data_item, auxiliary_images)
...
class StreamPackedDatasetMTP(IterableDataset):
    ...
    def __init__(
        ...
        self.max_samples_per_pack = max_samples_per_pack
        ...
        if get_rank() == 0:
            ...
            logger.info(f'StreamPackedDatasetMTP initialized:\n'
                       ...
                       f'  max_samples_per_pack={max_samples_per_pack}\n'
    ...
    def __iter__(self):
        ...
        while True:
            ...
            pack_is_full = (
                self.max_samples_per_pack > 0
                and len(current_batch_locations) >= self.max_samples_per_pack
            )
            remaining_space = (
                -1 if pack_is_full else self.max_num_tokens - current_len
            )
...
def packed_collate_fn_mtp(features: List[dict], dataset: Optional[StreamPackedDatasetMTP] = None) -> dict:
    ...
    result = dict(
        ...
        image_flags=image_flags,
        modality_ids=feat['modality_ids'],
        modality_group_sizes=feat['modality_group_sizes'],
...
class EnforceSaveIntervalCallback(TrainerCallback):
    """Make resumed TrainerState follow the current save_steps argument."""

    def on_train_begin(self, args, state, control, **kwargs):
        state.save_steps = int(args.save_steps)
        logger.info(f"Checkpoint interval set to {state.save_steps} steps")
        return control
...
class StreamPackingMTPTrainer(Trainer):
    ...
    def __init__(self, *args, sample_log_interval: int = 100, **kwargs):
        ...
        self.save_trainable_only = False
    ...
    def _save(self, output_dir=None, state_dict=None):
        ...
        torch.save(
            trainable_state,
            os.path.join(output_dir, "trainable_state.pt"),
        )
        ...
        logger.info(
            f"Saved {len(trainable_state)} trainable tensors to {output_dir}"
        )

    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        trainable_path = os.path.join(
            resume_from_checkpoint, "trainable_state.pt"
        )
        ...
        trainable_state = torch.load(
            trainable_path, map_location="cpu", weights_only=True
        )
        incompatible = target_model.load_state_dict(trainable_state, strict=False)
        if incompatible.unexpected_keys:
            raise ValueError(
                f"Unexpected trainable checkpoint keys: "
                f"{incompatible.unexpected_keys}"
            )
        logger.info(
            f"Loaded {len(trainable_state)} trainable tensors from "
            f"{trainable_path}"
        )
...
def build_stream_packed_dataset_mtp(
    ...
    for ds_name, meta in ds_collections.items():
        ...
        try:
            ds = LazySupervisedDatasetMTP(
                ...
                use_modality_fusion=model_args.use_modality_fusion,
                modality_dropout_prob=model_args.modality_dropout_prob,
            )
    ...
    return StreamPackedDatasetMTP(
        ...
        max_samples_per_pack=getattr(data_args, 'max_samples_per_pack', 0),
        log_freq=10000,
        base_seed=base_seed,
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        config.use_modality_fusion = model_args.use_modality_fusion
        config.fusion_bottleneck_dim = model_args.fusion_bottleneck_dim
        config.fusion_window_size = model_args.fusion_window_size
        config.fusion_max_residual_scale = model_args.fusion_max_residual_scale
        logger.info(f'Text attn: {model_args.attn_implementation}, Vision attn: flash_attention_2')
        logger.info(
            f'Modality fusion: enabled={model_args.use_modality_fusion}, '
            f'bottleneck={model_args.fusion_bottleneck_dim}, '
            f'window={model_args.fusion_window_size}, '
            f'max_residual_scale={model_args.fusion_max_residual_scale}, '
            f'dropout={model_args.modality_dropout_prob}'
        )
        ...
        if model_args.use_modality_fusion:
            model.reset_modality_fusion_parameters()
            if model_args.modality_fusion_path:
                fusion_path = model_args.modality_fusion_path
                if osp.isdir(fusion_path):
                    fusion_path = osp.join(fusion_path, "trainable_state.pt")
                fusion_state = torch.load(fusion_path, map_location="cpu", weights_only=True)
                incompatible = model.load_state_dict(fusion_state, strict=False)
                logger.info(
                    f"Loaded modality fusion state from {fusion_path}; "
        ...
        locateanything_config = LocateAnythingConfig(
            ...
            use_modality_fusion=model_args.use_modality_fusion,
            fusion_bottleneck_dim=model_args.fusion_bottleneck_dim,
            fusion_window_size=model_args.fusion_window_size,
            fusion_max_residual_scale=model_args.fusion_max_residual_scale)
    ...
    # Callbacks
    my_callbacks = []
    if model_args.save_every_n_hours > 0 and not model_args.save_trainable_only:
    ...
    my_callbacks.append(EnforceSaveIntervalCallback())
    if not model_args.save_trainable_only:
    ...
    trainer.save_trainable_only = model_args.save_trainable_only
    ...
    if training_args.do_train:
        ...
        if model_args.save_trainable_only:
            if get_rank() == 0:
                ...
                torch.save(
                    trainable_state,
                    osp.join(training_args.output_dir, "trainable_state.pt"),
                )
                model.config.save_pretrained(training_args.output_dir)
                processor.save_pretrained(training_args.output_dir)
                logger.info(
                    f"Saved {len(trainable_state)} trainable tensors to trainable_state.pt"
                )
            dist.barrier()
        else:
            trainer.save_model()
