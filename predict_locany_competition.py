#!/usr/bin/env python3
"""Generate normalized LocateAnything predictions for the AIC query set."""
...
def draw_result(
    ...
    status = "fallback" if used_fallback else "model prediction"
    label = f"Pred red | {status}\n{query}"
    ...
    with path.open("r", encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            try:
                ...
                print(f"Ignoring incomplete progress line {line_number}", flush=True)
                break
...
def main() -> None:
    ...
    pending = [(index, row) for index, row in enumerate(rows) if row[0] not in completed]
    print(
        f"Loaded {len(completed)} completed predictions; "
        f"{len(pending)} of {len(rows)} remain.",
        flush=True,
    )
    ...
    for index, (sample_id, item) in pending:
        ...
        for attempt in range(args.parse_retries + 1):
            attempts = attempt + 1
            sample_seed = args.seed + index * 1009 + attempt
        ...
        if prediction is None:
            prediction = fallback_box.copy()
        prediction = [round(max(0.0, min(1.0, float(value))), 6) for value in prediction]
        result = {
            "id": sample_id,
            "bbox": prediction,
            "answer": answer,
        ...
        if len(visualizations) < args.visualize_count:
            rendered = draw_result(rgb, query, prediction, used_fallback)
        ...
        print(
            ...
            f"bbox={prediction} fallback={used_fallback} attempts={attempts}",
            flush=True,
        )
    progress_file.close()

    source_data = dict(load_annotations(args.annotation))
    output_data = {}
    for sample_id, item in rows:
        output_item = dict(source_data[sample_id])
        output_item["bbox"] = completed[sample_id]["bbox"]
        output_data[sample_id] = output_item
    output_json = args.output_json or args.output_dir / f"queries_{args.mode}.json"
    atomic_json_dump(output_data, output_json)

    total = len(rows)
    fallback_count = sum(bool(completed[sample_id]["used_fallback"]) for sample_id, _ in rows)
    summary = {
        "mode": args.mode,
        "seed": args.seed,
        "samples": total,
        "model_predictions": total - fallback_count,
        "fallback_predictions": fallback_count,
        "valid_rate": (total - fallback_count) / total if total else 0.0,
